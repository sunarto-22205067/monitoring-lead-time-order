        ...
        <div :class="['flex-1 flex flex-col min-w-0 overflow-hidden transition-all duration-300', sidebarOpen ? 'lg:ml-72' : 'ml-0']">
            ...
            <header class="bg-white border-b border-slate-100 px-4 lg:px-8 py-5 flex items-center gap-5 sticky top-0 z-10">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-red-50 text-red-700 rounded-lg text-xs font-bold border border-red-100">
                    ...
                    <div class="relative">
                        <button @click="userMenuOpen = !userMenuOpen"
                            class="flex items-center gap-2.5 p-1.5 pr-3.5 rounded-full border border-slate-200 bg-white hover:bg-slate-50 hover:shadow-sm transition-all focus:outline-none focus:ring-2 focus:ring-[#EE2E24]/20">
                            <img :src="user.profile_photo_url" :alt="user.name" class="w-8 h-8 rounded-full object-cover"/>
                            <div class="min-w-0 hidden md:block text-left mr-1">
                                <p class="text-[12px] font-bold text-slate-800 leading-tight">{{ user.name }}</p>
                            </div>
                            <svg class="w-4 h-4 text-slate-400 flex-shrink-0 hidden md:block transition-transform duration-200" :class="{'rotate-180': userMenuOpen}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <transition 
                            ...
                            <div v-if="userMenuOpen" class="absolute top-full right-0 mt-3 w-64 bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden z-50">
                                <!-- Dropdown Header -->
                                <div class="px-5 py-4 bg-slate-50/80 border-b border-slate-100 flex items-center gap-3">
                                    <img :src="user.profile_photo_url" :alt="user.name" class="w-10 h-10 rounded-full object-cover ring-2 ring-white shadow-sm"/>
                                ...
                                <div class="p-2 space-y-0.5">
                                    <Link :href="route('profile.edit')" class="group w-full flex items-center gap-3 px-3 py-2 text-[13px] font-semibold text-slate-700 rounded-xl hover:bg-slate-50 hover:text-[#EE2E24] transition-colors">
                                        <div class="w-7 h-7 rounded-lg bg-slate-100 flex items-center justify-center text-slate-500 group-hover:text-[#EE2E24] group-hover:bg-red-50 transition-colors">
                                    ...
                                    <button @click="logout" class="w-full flex items-center gap-3 px-3 py-2 text-[13px] font-semibold text-red-600 rounded-xl hover:bg-red-50 transition-colors text-left">
